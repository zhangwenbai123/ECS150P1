...
int main(void)
{
        ...
        while (1) {
                ...
                for (int i = 0; i < numPipeInstructions; ++i) {
                        ...
                        pipeOfCmds.cmds[i].isAppend = false;
                ...
                for (int i = 0; i < pipeOfCmds.numPipeInstructions; ++i) {
                        ...
                        if (pipeOfCmds.cmds[i].isRedirect || pipeOfCmds.cmds[i].isAppend) {
                                if (pipeOfCmds.cmds[i].isRedirect) {
                                        ...
                                        fd = open(pipeOfCmds.cmds[i].redirectLoc, O_APPEND | O_WRONLY | O_CREAT,0644);
                                        if (fd < 0) {
                                                fprintf(stderr, "Error: cannot open output file\n");
                                                continue;
                                        }
                ...
                fprintf(stderr, "+ completed '%s' ", inputCopy);
                for (int i = 0; i < pipeOfCmds.numPipeInstructions; ++i) {
                        fprintf(stderr, "[%d]", WEXITSTATUS(returnStatus[i]));
                }
                fprintf(stderr, "\n");
